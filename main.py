...
def train(args, return_early=False):
    ...
    for j in range(args.num_updates):
        for step in range(args.num_steps):
            ...
            #print('agent_action:{}'.format(type(agent_actions)))
        ...
        total_reward.append(final_rewards.mean(dim=0).cpu().numpy()[0])
        ...
        if args.eval_interval is not None and j%args.eval_interval==0:
            ...
            ob_rms = (None, None) if envs.ob_rms is None else (envs.ob_rms[0].mean, envs.ob_rms[0].var)
            print('===========================================================================================')
            _, eval_perstep_rewards, final_min_dists, num_success, eval_episode_len = evaluate(args, None, master.all_policies,render=True,
            ...
            print('Num success {:d}/{:d} | Episode Length {:.2f} | EX_extra_time {:.2f} | Std_extra_time {:.2f}'.format(num_success, args.num_eval_episodes, eval_episode_len[0], eval_episode_len[1], eval_episode_len[2]))
            print('human Num success {:d}/{:d} | Episode Length {:.2f} | human_EX_extra_time {:.2f} | human_Std_extra_time {:.2f}'.format(eval_episode_len[3], args.num_eval_episodes, eval_episode_len[0], eval_episode_len[4], eval_episode_len[5]))
            if final_min_dists:
            ...
            if not args.test:
                ...
                writer.add_scalar('all/episode_length', eval_episode_len[0], j)
                for idx in range(n):
