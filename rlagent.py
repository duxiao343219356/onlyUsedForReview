from RL_algorithm.meppo import PPO
from RL_algorithm.storage import RolloutStorage
...
class Agent(object):
  ...
  def update_rollout(self, obs, reward, mask):
    # import pdb
    # pdb.set_trace()
    reward = reward + self.dist_entropy  #add for meppo
    self.rollouts.insert(obs, self.states, self.action, self.action_log_prob, self.value, reward, mask)

  def act(self, step, deterministic=False):#add entropy for meppo
    self.value, self.action, self.action_log_prob, self.states, self.dist_entropy= self.actor_critic.act(self.rollouts.obs[step],
