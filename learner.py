import numpy as np
import torch
from RL_algorithm import JointPPO
from rlagent import Agent
from state_presentation import MPNN
...
def setup_master(args, env=None, return_env=False):
    ...
    num_entities = env.num_robots
    num_human =  env.human_num
    num_robots = env.num_robots

    if entity_mp:
        # pol_obs_dim = env.observation_space[0].shape[0] - 2*num_entities - 4*num_human
        # pol_obs_dim = env.observation_space[0].shape[0] - 2 - 4*num_human
        pol_obs_dim = 4
    else:
        pol_obs_dim = env.observation_space[0].shape[0]

    # index at which agent's position is present in its observation
    pos_index = args.identity_size
    # import pdb
    # pdb.set_trace()
    for i, agent in enumerate(env.robots):
        obs_dim = env.observation_space[i].shape[0]
        if hasattr(agent, 'adversary') and agent.adversary:
            if policy1 is None:
                policy1 = MPNN(input_size=pol_obs_dim,num_agents=num_adversary,num_entities=num_entities,num_human=num_human,action_space=action_space,
                               pos_index=pos_index, mask_dist=args.mask_dist,entity_mp=entity_mp,obs_robot_dim=env.obs_robot_dim).to(args.device)
            team1.append(Agent(args,policy1,(obs_dim,),action_space))
        else:
            if policy2 is None:
                policy2 = MPNN(input_size=pol_obs_dim,num_agents=num_friendly,num_entities=num_entities,num_human=num_human,action_space=action_space,
                               pos_index=pos_index, mask_dist=args.mask_dist,entity_mp=entity_mp,obs_robot_dim=env.obs_robot_dim).to(args.device)
            team2.append(Agent(args,policy2,(obs_dim,),action_space))
...
class Learner(object):
    ...
    def act(self, step):
        ...
        for team, policy in zip(self.teams_list, self.policies_list):
            ...
            # split all outputs
            n = len(team)
            all_value, all_action, all_action_log_prob, all_states, all_dist_entropy = [torch.chunk(x, n) for x in props]
            for i in range(n):
                team[i].value = all_value[i]
                team[i].action = all_action[i]
                team[i].action_log_prob = all_action_log_prob[i]
                team[i].states = all_states[i]
                team[i].dist_entropy = all_dist_entropy[i]  #add for meppo
    ...
    def eval_act(self, obs, recurrent_hidden_states, mask):
        ...
        for team,policy,obs in zip(self.teams_list,self.policies_list,all_obs):
            if len(obs)!=0:
                _,action,_,_,_ = policy.act(torch.cat(obs).to(self.device),None,None,deterministic=True)
