...
import numpy as np
from rlcore.discrete_distributions import Categorical
import torch.nn.functional as F
import math
from graph_conv_model import RGL
...
class MPNN(nn.Module):
    def __init__(self, action_space, num_agents, num_entities, num_human, input_size=16, hidden_dim=128, embed_dim=None,
                 pos_index=2, norm_in=False, nonlin=nn.ReLU, n_heads=1, mask_dist=None, entity_mp=False, obs_robot_dim=0):
        super().__init__()

        # Initialization graph convolution model
        device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
        self.gcn_layer_number = 2
        self.human_gcn_model = RGL(self.gcn_layer_number)
        self.robot_gcn_model = RGL(self.gcn_layer_number)
        self.human_gcn_model.to(device)
        self.robot_gcn_model.to(device)
      
        self.h_dim = hidden_dim
        self.nonlin = nonlin
        self.num_agents = num_agents # number of agents
        self.num_entities = num_entities # number of entities
        self.obs_robot_dim = obs_robot_dim
        self.num_human = num_human
        self.K = 2 # message passing rounds
        ...
        self.messages_origin = MultiHeadAttention(n_heads=self.n_heads,input_dim=self.h_dim,embed_dim=self.embed_dim)
        ...
        if self.entity_mp:
            ...
            self.entity_land_update = nn.Sequential(nn.Linear(2*self.h_dim+self.embed_dim,self.h_dim),
                                               self.nonlin(inplace=True))
        self.human_encoder = nn.Sequential(nn.Linear(4,self.h_dim),
                                           self.nonlin(inplace=True))
        self.obs_robot_encoder = nn.Sequential(nn.Linear(2,self.h_dim),
                                           self.nonlin(inplace=True))
        self.human_messages = MultiHeadAttention(n_heads=1,input_dim=self.h_dim,embed_dim=self.embed_dim)
        self.human_robot_messages = MultiHeadAttention(n_heads=1,input_dim=self.h_dim,embed_dim=self.embed_dim)

        num_actions = action_space.n
        # print('action_space.n:{}'.format(num_actions))
    ...
    def _fwd(self, inp):
        ...
        mask = self.compute_mask(agent_inp) 
       
        h = self.encoder(agent_inp) # should be (batch_size,self.h_dim)
        landmark_size = self.input_size + 4*self.num_human
        if self.entity_mp:
            landmark_inp = inp[:,landmark_size:landmark_size+2]  # x,y pos of landmarks wrt robot
        ...
        # obstacle GCN
        if self.human_mp:
            human_inp = inp[:,self.input_size:landmark_size]
            he = self.human_encoder(human_inp.contiguous().view(-1,4)).view(-1,self.num_human,self.h_dim)
            human_message = self.human_gcn_model(torch.cat([h.unsqueeze(1), he],1))[:,0,:]
            # human_message = self.human_messages(h.unsqueeze(1), he).squeeze(1)

        # robot observation GCN
        robot_origin = self.obs_robot_encoder(inp[:,landmark_size+2:].contiguous().view(-1,2)).view(-1,self.obs_robot_dim,self.h_dim) 
        obs_robot_message = self.human_gcn_model(torch.cat([h.unsqueeze(1), robot_origin],1))[:,0,:]



        #iter-group
        human_robot_message = torch.cat((obs_robot_message, human_message),1).view(-1,2,self.embed_dim)
        all_message = self.human_robot_messages(h.unsqueeze(1),human_robot_message).squeeze(1)
        
        h = self.entity_land_update(torch.cat((torch.cat((h,all_message),1),entity_message),1))
        
        h = h.view(self.num_agents,-1,self.h_dim).transpose(0,1) #  2*3*128 should be (batch_size/N,N,self.h_dim)
    ...
    def act(self, inp, state, mask=None, deterministic=False):
        ...
        if deterministic:
            ...
            action = dist.sample()
        dist_entropy_s = dist.entropy() # add for meppo  
        dist_entropy = torch.unsqueeze(-0.01*dist_entropy_s,1) #add for meppo
        action_log_probs = dist.log_probs(action).view(-1,1)
        return value,action,action_log_probs,state,dist_entropy 
